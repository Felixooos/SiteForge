      ...
      podiumOrder.forEach((player, i) => {
        ...
        html += `
          <div class="lb-podium-item" data-email="${escAttr(player.email)}">
            ...
            <div class="lb-podium-name ${player.is_creator ? 'name-creator' : (player.is_admin || player.is_super_admin) ? 'name-admin' : ''}">${escHtml(player.pseudo || player.email)}${player.is_creator ? '<span class="role-tag role-tag-creator">CREATEUR</span>' : (player.is_admin || player.is_super_admin) ? '<span class="role-tag role-tag-admin">ADMIN</span>' : ''}</div>
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      html += `
        <div class="lb-row ${isMe ? 'lb-row-me' : ''}" data-email="${escAttr(player.email)}">
          ...
          <div class="lb-info">
            <div class="lb-name ${player.is_creator ? 'name-creator' : (player.is_admin || player.is_super_admin) ? 'name-admin' : ''}">${escHtml(player.pseudo || player.email)}${player.is_creator ? '<span class="role-tag role-tag-creator">CREATEUR</span>' : (player.is_admin || player.is_super_admin) ? '<span class="role-tag role-tag-admin">ADMIN</span>' : ''}</div>
