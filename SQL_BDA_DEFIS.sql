...
DROP POLICY IF EXISTS "bda_defis_select" ON bda_defis;
CREATE POLICY "bda_defis_select" ON bda_defis FOR SELECT USING (true);

-- Admins can insert
DROP POLICY IF EXISTS "bda_defis_admin_insert" ON bda_defis;
...
-- Admins can update
DROP POLICY IF EXISTS "bda_defis_admin_update" ON bda_defis;
CREATE POLICY "bda_defis_admin_update" ON bda_defis FOR UPDATE USING (
  EXISTS (SELECT 1 FROM etudiants WHERE email = auth.jwt() ->> 'email' AND site_id = bda_defis.site_id AND (is_admin = TRUE OR is_super_admin = TRUE))
);

-- Admins can delete
DROP POLICY IF EXISTS "bda_defis_admin_delete" ON bda_defis;
